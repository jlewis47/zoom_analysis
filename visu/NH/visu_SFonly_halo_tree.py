...
from hagn.catalogues import get_nh_cats_h5, convert_cat_units, get_cat_hids, make_super_cat
...
for istep, (aexp, time) in enumerate(zip(nh_tree_aexps, nh_tree_times)):
    ...
    try:
        super_cat = make_super_cat(
            nh_snap, outf="/data101/jlewis/nh/super_cats", sim="nh"
        , overwrite=True)
        print(istep, 1./aexp-1)
    ...
    if len(gal_pties['mgal'])==0:continue
...
for istep, (snap, aexp, time) in enumerate(zip(snaps, sim_aexps, sim_times)):
    ...
    fouts=[]
    keys=["dens","stmass","sfr","age","temp",'sfrVmstel']

    for iout in range(6):
        fouts.append(os.path.join(
            outdir,
            f"{keys[iout]}_{snap:04d}_{plot_win_str}{rstr}{option_str:s}.png",
        ))
    ...
    # fig,axs = plt.subplots(2,3,figsize=(30,30),width_ratios=[1,1,1],height_ratios=[1,1], layout="constrained")
    ...
    stars_in_3r50 = np.linalg.norm(stars["pos"] - tgt_pos[None,:], axis=1) < 3 * cur_r50
    ...
    sfrs100 = stmasses[stages<100.]/100.

    mass_in_3r50 = np.sum(stmasses[stars_in_3r50])
    sfr100_in_3r50 = np.sum(stmasses[stars_in_3r50][stages[stars_in_3r50]<100]/100.)
    ...
    for arg in [dens_args, smass_args, stage_arg, sfr_arg, temperature_args, vel_args, mach_args, alpha_vir_args]:
        arg['hid']=cur_hid

    img_dens = make_amr_img_smooth(fig, ax_dens, 'density', snap, sim, tgt_pos, rad_tgt, zdist, NH_read=True, **dens_args)
    img_temp = make_amr_img_smooth(fig, ax_temp, 'temperature', snap, sim, tgt_pos, rad_tgt, zdist, NH_read=True, **temperature_args)
    ...
    for iax,ax in enumerate(axs[:-1]):
        ax.text(
            ...
            0.9,
            "z = %.2f" % zed,
            color=arg_dicts[iax]["marker_color"],
    ...
    for fig,fout in zip(figs,fouts):
