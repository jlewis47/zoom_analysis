...
from zoom_analysis.catalogue.read_cat import read_cat, obj_based_to_table
...
for isim, sim_dir in enumerate(sim_dirs):
    ...
    # sfr = gal_dict["sfr100"]/1e6 #Msun/yr
    sfr = obj_based_to_table(gal_dict, "stars", 'sfr100_Msun_per_yr')

    print(f"I found {len(gal_pos):d} galaxies with purity > {fpure:f},")
    ...
    bin_centers, bin_edges, binnumber = binned_statistic(
        gal_mass, gal_mass, bins=stellar_bins, statistic="mean"
    )
...
print(len(vol_args), vol_args.sum(), min_vol)
