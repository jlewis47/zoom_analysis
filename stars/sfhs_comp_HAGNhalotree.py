...
def follow_hagn_halo(
    ...
    for inode, node_aexp in enumerate(hagn_tree_aexps):
        ...
        # print(os.listdir(sim.path))

        hagn_ctr = decentre_coordinates(hagn_ctr, sim.path)

        # print(hagn_rvir)
        # print(hagn_ctr, old_ctr)
...
sdirs = ["/data101/jlewis/sims/dust_fid/lvlmax_20/mh1e12" for entry in sim_ids]
...
# # add 2nd yaxis to first plot
# y2 = ax[0].twiny()
# # make it redshift
# y2.set_xlim(ax[0].get_xlim())
# z_ticks = np.asarray(
#     [
#         1.0 / hagn_aexps[np.argmin(np.abs(tick_t - hagn_times / 1e3))] - 1.0
#         for tick_t in ax[-1].get_xticks()
#     ]
# )
# y2.set_xticks(ax[-1].get_xticks())
# y2.set_xticklabels([f"{z:.1f}" for z in z_ticks])
...
zticks = [
    z_at_value(hagn_sim.cosmo_model.age, (x + 1e-3) * u.Gyr).value for x in xticks
]
ax2.set_xticklabels(["{:.2f}".format(z) for z in zticks])


fig.savefig("sfhs_comp_HAGNhalotree.png")
