...
stellar_bins = np.logspace(9, 13, 6)
...
for isim, sim_dir in enumerate(sim_dirs):
    ...
    ax_ratio.plot(
        bin_centers, bin_mean_ratios, color=points.get_facecolor(), ls="-", alpha=1.0
    )
...
if has_hagn_snap:
    ...
    if vol_args.sum() > 0:
        ...
        ax.scatter(
            mhalos, mgals, alpha=0.5, marker="x", color="k"
        )  # , label=f"HAGN, z={hagn_zed:.2f}")
        ax_ratio.scatter(
            mhalos, mgals / mhalos, alpha=0.5, marker="x", color="k"
        )  # , label=f"HAGN, z={hagn_zed:.2f}")
        ...
        ax_ratio.plot(
            med_bin, avg_ratio, c="k", lw=1, ls="--", label=f"HAGN, z={hagn_zed:.2f}"
        )
