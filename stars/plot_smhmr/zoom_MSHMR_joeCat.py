...
import h5py
from zoom_analysis.catalogue.read_cat import obj_based_to_table, read_cat
...
stellar_bins = np.logspace(8, 13, 12)
...
for isim, sim_dir in enumerate(sim_dirs):
    ...
    gal_dict = read_cat(sim.path, sim_snap, "galaxy", "reff", 1.0)
    # print(gal_dict)
    vol_args,min_vol = check_in_all_sims_vol(gal_dict["pos"], sim, sim_dirs)

    host_purity = gal_dict["host purity"]
    # central_cond = gal_dict["central"] == 1
    # pure_cond = (host_purity > fpure) * central_cond * vol_args
    # pure_cond = (host_purity > fpure)  * vol_args
    # gal_mass = gal_dict["mass"][pure_cond]
    gal_mass = obj_based_to_table(gal_dict, "stars", 'Mstar_msun')
    gal_pos = gal_dict["pos"][:, :]
    # rmax = gal_dict["rmax"][pure_cond]
    halo_mass = gal_dict["host mass"]

    print(f"I found {len(gal_dict):d} galaxies with purity > {fpure:f},")
    ...
    points = ax.scatter(
        halo_mass, gal_mass, label=label, alpha=0.5, marker=markers[isim]
    )
    points = ax_ratio.scatter(
        halo_mass, gal_mass/halo_mass, label=label, alpha=0.5, marker=markers[isim]
    )

    # get avg values in bins
    bin_means, bin_edges, binnumber = binned_statistic(
        halo_mass, gal_mass, bins=stellar_bins, statistic="mean"
    )
    bin_mean_ratios, bin_edges, binnumber = binned_statistic(
        halo_mass, gal_mass/halo_mass, bins=stellar_bins, statistic="mean"
    )
    bin_centers, bin_edges, binnumber = binned_statistic(
        halo_mass, halo_mass, bins=stellar_bins, statistic="median"
    )
    ...
    ax_ratio.plot(bin_centers, bin_mean_ratios, color=points.get_facecolor(), ls="-", alpha=1.0)
...
if vol_args.sum() > 0:
    ...
    ax_ratio.scatter(mhalos, mgals/mhalos, alpha=0.5, marker="x",color='k')#, label=f"HAGN, z={hagn_zed:.2f}")
    ...
    ax_ratio.plot(med_bin, avg_ratio, c="k", lw=1, ls="--",label=f"HAGN, z={hagn_zed:.2f}")    
...
ax.set_xlabel("Halo mass Mass [M$_\odot$]")
ax_ratio.set_ylabel("Stellar mass [M$_\odot$]")
ax_ratio.set_xlabel("Halo mass Mass [M$_\odot$]")
...
ax.plot(xrange, xrange * 0.1, ls="--", color="black", label="10%")
ax_ratio.plot(xrange, np.full_like(xrange,0.1), ls="--", color="black", label="10%")

ax.legend(framealpha=0.0, title=title_txt)
ax_ratio.legend(framealpha=0.0, title=title_txt)

fig_name = "zoom_SMHMR_JC"
...
if tgt_zed != None:
    ...
    fig_ratio.savefig(f"{fig_name:s}_z{tgt_zed:.2f}_ratio.png")
else:
    fig.savefig(f"{fig_name:s}.png")
    fig_ratio.savefig(f"{fig_name:s}_ratio.png")
