...
for sdir in sdirs:
    ...
    mstars = np.zeros(nsteps)
    disps = np.zeros((nsteps, 2))
    rots = np.zeros((nsteps, 2))
    mdots_coarse = np.zeros(nsteps)
    mdots_torque = np.zeros((nsteps, 4))
    mdots_edd = np.zeros(nsteps)
    sfrs = np.zeros(nsteps)
    fdisks = np.zeros((nsteps, 2))
    mdisks = np.zeros((nsteps, 2))
    mbulges = np.zeros((nsteps, 2))
    ...
    for istep, (snap, aexp, time) in enumerate(
        ...
        print(
            f"found {len(star_args):d} stars and {len(gas_args):d} gas particles within {rmax_pc:.1f} pc"
        )

        if stars is None or gas is None:
        ...
        bulk_vel_stars = np.average(stars["vel"], weights=stars["mass"], axis=0)
        ...
        gas_rot_props, gas_kin_props = extract_nh_kinematics(
            gas_masses, gas_pos, gas_vel - bulk_vel_gas, tgt_pos, debug=False
        )
        star_rot_props, star_kin_props = extract_nh_kinematics(
            masses, stars["pos"], stars["vel"] - bulk_vel_stars, tgt_pos, debug=False
        )

        gas_vrot = gas_rot_props["Vrot"]
        star_vrot = star_rot_props["Vrot"]
        gas_disp = gas_rot_props["disp"]
        star_disp = star_rot_props["disp"]
        gas_fbulge = gas_kin_props["fbulge"]
        star_fbulge = star_kin_props["fbulge"]
        gas_mdisk = gas_kin_props["Mdisk"]
        star_mdisk = star_kin_props["Mdisk"]
        gas_fdisk = gas_mdisk / (gas_masses.sum() + masses.sum())
        star_fdisk = star_mdisk / (gas_masses.sum() + masses.sum())

        # print(f"vrot/disps={vrot/disp}")
        # print(f"vrot={vrot}, disp={disp}")

        disps[istep] = star_disp, gas_disp
        rots[istep] = star_vrot, gas_vrot
        mdisks[istep] = star_mdisk, gas_mdisk
        mbulges[istep] = (star_mdisk / star_fdisk) * star_fbulge, (
            gas_mdisk / gas_fdisk
        ) * gas_fbulge
        fdisks[istep] = star_fdisk, gas_mdisk

        print(
            f"STARS:f_bulge={star_fbulge}, f_disk={star_fdisk}",
            f"m_disk={star_mdisk:.1e}",
        )
        print(
            f"GAS:f_bulge={gas_fbulge}, f_disk={gas_fdisk}", f"m_disk={gas_mdisk:.1e}"
        )

        rmax_pc = tgt_r * sim.cosmo.lcMpc * 1e6 * aexp

        # print(
        #     f"eta_bh: {0.1}, alphaT: {5}, fdisk: {fdisk:.1f}, Mbh: {sink['mass']:.1e}, Mdisk: {mdisk:.1e}, Mgas: {gas_masses.sum():.1e}, R0: {rmax_pc:.1e}"
        # )

        star_mdot_torque = torque_limited(
            0.1,
            5,
            star_fdisk,
            sink["mass"],
            star_mdisk + gas_mdisk,
            gas_masses.sum(),
            rmax_pc,
        )
        print(f"STARS:mdot_torque={star_mdot_torque:.1e}")
        gas_mdot_torque = torque_limited(
            0.1,
            5,
            gas_fdisk,
            sink["mass"],
            gas_mdisk + star_mdisk,
            gas_masses.sum(),
            rmax_pc,
        )
        print(f"GAS:mdot_torque={gas_mdot_torque:.1e}")

        tot_m = gas_masses.sum() + masses.sum()
        tot_fdisk = star_fdisk + gas_fdisk

        tot_mdot_torque = torque_limited(
            0.1,
            5,
            tot_fdisk,
            sink["mass"],
        ...
        print(f"TOTAL :mdot_torque={tot_mdot_torque:.1e}")
        print(f"TOTAL (sum):mdot_torque={gas_mdot_torque+star_mdot_torque:.1e}")
        ...
        mdots_torque[istep] = (
            ...
            gas_mdot_torque,
            tot_mdot_torque,
            gas_mdot_torque + star_mdot_torque,
        )
        mdots_coarse[istep] = sink["dMBH_coarse"]
        mdots_edd[istep] = sink["dMEd_coarse"]

        SFR = masses[ages < 10].sum() / 10e6  # Msun/yr

        sfrs[istep] = SFR

        # print(f"SFR={SFR:.1e}")

        # print(f"Mdot_torque/SFR={mdot_torque/SFR:.1e}")
    ...
    plot_dir = os.path.join(".", "test_trq_ltd_dual", sim.name)
    if not os.path.exists(plot_dir):
        os.makedirs(plot_dir)

    non_zero = np.where((mstars > 0) * (mdots_coarse > 0))[0]
    plot_times = sim_times[::-1][non_zero]
    # plot_times = sim_times[non_zero]
    ...
    ax1.plot(plot_times, mstars[non_zero], label="Stellar Mass")
    ax1.plot(
        plot_times, mbulges[non_zero], label=["Stars bulge_mass", "Gas bulge_mass"]
    )
    ax1.plot(plot_times, mdisks[non_zero], label=["Stars disk mass", "Gas disk mass"])
    ...
    ax2.plot(plot_times, disps[non_zero], label=["Stars dispersion", "Gas dispersion"])
    ax2.plot(
        plot_times, np.abs(rots[non_zero]), label=["Stars rotation", "Gas rotation"]
    )
    ...
    ax3.plot(
        plot_times,
        mdots_torque[non_zero],
        label=["Stars only", "Gas only", "Torque-limited", "gas+stars"],
    )
    ax3.plot(plot_times, mdots_coarse[non_zero], label="Bondi")
    ax3.plot(
        plot_times,
        mdots_edd[non_zero],
    ...
    ax4.plot(
        plot_times,
        fdisks[non_zero],
        label=["Stars bulge fraction", "Gas bulge fraction"],
    )
    ax4.plot(
        plot_times,
        1 - fdisks[non_zero],
        label=["Stars disk fraction", "Gas disk fraction"],
    )
    ...
    ax5.plot(plot_times, sfrs[non_zero], label="Star Formation Rate")
    ax5.set_ylabel("Star Formation Rate, $M_{\odot}/yr$")
    ax5.set_yscale("log")
    ax5.set_xlabel("Time, $Myr$")
    ax5.legend()
    plt.tight_layout()
    fig5.savefig(os.path.join(plot_dir, "star_formation_rate.png"))

    fig6, ax6 = plt.subplots()
    ax6.plot(
        plot_times,
        mdots_torque[non_zero] / sfrs[non_zero, np.newaxis],
        label="Mdot_torque / SFR",
    )
    ...
    # arg_first_mbh_mass = np.argmin(mbhs[non_zero])
    ...
    #         mdots_torque[non_zero][::-1][:istep], sim_times[non_zero][:istep] * 1e6
    #     )
    #     intrgr8td_bondi[istep] += np.trapz(
    #         mdots_coarse[non_zero][::-1][:istep], sim_times[non_zero][:istep] * 1e6
    #     )
    #     intgr8td_edd[istep] += np.trapz(
    #         mdots_edd[non_zero][::-1][:istep], sim_times[non_zero][:istep] * 1e6
    #     )
    ...
    # intrgr8td_bondi[:arg_first_mbh_mass] += mbhs[non_zero][arg_first_mbh_mass]
    # intgr8td_mdot_trq[:arg_first_mbh_mass] += mbhs[non_zero][arg_first_mbh_mass]
    # intgr8td_edd[:arg_first_mbh_mass] += mbhs[non_zero][arg_first_mbh_mass]

    # # intgr8td_mdot_trq = np.trapz(mdots_torque[non_zero], plot_times * 1e6)
    # # intgr8td_edd = np.trapz(mdots_edd[non_zero], plot_times * 1e6)
    # # intrgr8td_bondi = np.trapz(mdots_coarse[non_zero], plot_times * 1e6)

    # fig9, ax9 = plt.subplots()

    # (l,) = ax9.plot(
    #     plot_times, mbhs[non_zero], label="Black Hole Mass (bondi)", ls="--"
    # )
    # ax9.plot(
    #     plot_times,
    #     intrgr8td_bondi,
