...
stellar_bins = np.logspace(6, 12, 8)
...
for isim, sim_dir in enumerate(sim_dirs):
    ...
    cur_hid = gal_dict['host hid'][pure_cond]
    ...
    zoom_VoverSig=np.zeros(np.sum(pure_cond))

    label = sim.name
    if len(sim_dirs) > 1:
        label += f", z={sim_zed:.4f}"

    for igal in range(pure_cond.sum()):

        try:
            gal_stars = read_data_ball(sim,sim_snap,gal_pos[:,igal],2*gal_dict['r50'][igal],host_halo=cur_hid[igal],data_types=['stars'],tgt_fields=["pos","vel","mass"])
        except (FileNotFoundError,AssertionError,KeyError):
            continue
            
        if gal_stars is None:continue

        rotation,_=extract_nh_kinematics(gal_stars["mass"],gal_stars["pos"],gal_stars["vel"],gal_pos[:,igal])

        zoom_VoverSig[igal] = abs(rotation['Vrot'])/rotation['disp']

        print(f"igal: {igal},V/sig: {zoom_VoverSig[igal]}")

    # print(sim.name,np.sum(vol_args))

    points = ax.scatter(
        gal_mass,zoom_VoverSig, label=label, alpha=0.5, marker=markers[isim]
    )

    # get avg values in bins
    bin_means, bin_edges, binnumber = binned_statistic(
        gal_mass, zoom_VoverSig, bins=stellar_bins, statistic="mean"
    )
    bin_mean_ratios, bin_edges, binnumber = binned_statistic(
        gal_mass, zoom_VoverSig, bins=stellar_bins, statistic="mean"
    )
    bin_centers, bin_edges, binnumber = binned_statistic(
        gal_mass, gal_mass, bins=stellar_bins, statistic="median"
    )
...
if has_hagn_snap:
    ...
    if vol_args.sum() > 0:
        ...
        VoverSig = hagn_gals['vrot'][vol_args]/hagn_gals['sigt'][vol_args]

        


        ax.scatter(mgals, VoverSig, alpha=0.5, marker="x",color='k')#, label=f"HAGN, z={hagn_zed:.2f}")
        

        #now avg stats
        avg, _, _ = binned_statistic(
            mgals,VoverSig, bins=stellar_bins, statistic=np.nanmean
        )
...
ax.set_ylabel(r"$\mathrm{V_{rot}/\sigma}$")
