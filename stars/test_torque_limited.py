...
for sdir in sdirs:
    ...
    for istep, (snap, aexp, time) in enumerate(
        ...
        pos_combined = np.concatenate([stars["pos"], gas_pos])
        ...
        vels_combined = np.concatenate(
            [stars["vel"] - bulk_vel_stars, gas_vel - bulk_vel_gas]
        )
        ...
        masses_combined = np.concatenate([masses, gas_masses])

        rot_props, kin_props = extract_nh_kinematics(
            masses_combined, pos_combined, vels_combined, tgt_pos, debug=False
        )
    ...
    plot_dir = os.path.join(".", "test_trq_ltd", sim.name)
    ...
    ax1.plot(plot_times, mbulges[non_zero], label="Bulge Mass")
    ax1.plot(plot_times, mdisks[non_zero], label="Disk Mass")
    ...
    ax2.plot(plot_times, stdisps[non_zero], label="Velocity Dispersion")
    ax2.plot(plot_times, np.abs(strots[non_zero]), label="Rotation Velocity")
    ...
    ax3.plot(plot_times, mdots_torque[non_zero], label="Torque-limited")
    ax3.plot(
        plot_times,
        mdots_edd[non_zero],
    ...
    ax4.plot(plot_times, fdisks[non_zero], label="Disk Fraction")
    ax4.plot(plot_times, 1 - fdisks[non_zero], label="Bulge Fraction")
    ...
    ax6.plot(
        plot_times, mdots_torque[non_zero] / sfrs[non_zero], label="Mdot_torque / SFR"
    )
