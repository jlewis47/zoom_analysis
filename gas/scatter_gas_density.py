...
for isim, sim_dir in enumerate(sim_dirs):
    ...
    gas_density = np.zeros(len(hids), dtype=np.float32)
    ...
    for igal in range(len(hids)):
        ...
        print(igal, np.log10(mstels[igal]), np.log10(gas_density[igal]))

    points = ax.scatter(
        mstels, gas_density, label=label, alpha=0.5, marker=markers[isim]
    )
    # points = ax.scatter(
    ...
    bin_means, bin_edges, binnumber = binned_statistic(
        mstels, gas_density, bins=stellar_bins, statistic="mean"
    )
    # bin_means, bin_edges, binnumber = binned_statistic(
...
ax.set_ylabel("Gas density,  [M$_\odot$/cMpc$^3$]")
