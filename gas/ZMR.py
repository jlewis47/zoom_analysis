...
for isim, sim_dir in enumerate(sim_dirs):
    ...
    ZOH = np.zeros(len(hids),dtype=np.float32)
    ZOHfromZ = np.zeros(len(hids),dtype=np.float32)
    ...
    for igal in range(len(hids)):
        ...
        try:
            datas = read_data_ball(sim, sim_snap, tgt_pos, tgt_r, hid,
                                data_types=["gas","stars"],
                                tgt_fields=["ilevel","density","chem_O","mass","age","metallicity"])
        except (KeyError,FileNotFoundError):
            continue 

        gas = datas['gas']
        if gas is None:continue
        if len(gas['ilevel'])<1:continue

        ZOH[igal] = np.average(gas['chem_O'],weights=gas['density'])

        Mgas = gas['density'] * ((sim.cosmo.lcMpc*ramses_pc*1e6)/2**gas['ilevel'])**3 #g/cc
        Zgas = gas['metallicity']

        #stolen from infinity:/home/dubois/myidl/nhroutines/scripts/plot_comp_gasmet.pro
        # ;; This is the mass fraction of Oxygen elements in Solar
        # ;; atmosphere (Asplund+09) ~43%. In principle, this fraction
        # ;; should depend on the metallicity (O/Z=function of Z)
        XHAsplund=0.7381
        XHeAsplund=0.2485
        lAO =8.69
        lAH =12.0
        logH =lAH                  +np.log10(XHAsplund)
        logO =lAO +np.log10(15.999)+np.log10(XHAsplund)
        nfracO=10**(lAO -lAH)
        mfracO=10**(logO-lAH)
        conv_factor=mfracO/(1.-(XHAsplund+XHeAsplund))
        zgO=conv_factor*Zgas
        mgO=np.sum(zgO*Mgas)
        mgHy=np.sum(XHAsplund*Mgas) # This also should be a function of Z
        ZOHfromZ[igal]=np.log10(mgO/mgHy)-(logO-logH)+lAO # 12+log(O/H)



        stars = datas['stars']
        if stars is None:continue
        if len(stars['mass'])<1:continue

        # print(stars)

        masses = sfhs.correct_mass(sim,stars["age"],stars["mass"],stars["metallicity"])

        mstels[igal] = np.sum(masses)

        print(igal,np.log10(mstels[igal]),np.log10(ZOH[igal])+12,ZOHfromZ[igal])


    points = ax.scatter(
        mstels, ZOHfromZ, label=label, alpha=0.5, marker=markers[isim]
    )
    # points = ax.scatter(
    ...
    bin_means, bin_edges, binnumber = binned_statistic(
        mstels, ZOHfromZ, bins=stellar_bins, statistic="mean"
    )
    # bin_means, bin_edges, binnumber = binned_statistic(
...
jades_zmr = plot_MZR.curti2024_MZR(tgt_zed,zed_tol=0.5)
jades_zmr_all = plot_MZR.curti2024_MZR(tgt_zed,zed_tol=np.inf)

ax.scatter(jades_zmr['Mstel'],jades_zmr['12 + log(O/H)'],label="Curti+24 3<z<10 (JADES)",
            color='k',marker="x",zorder=999)
ax.scatter(jades_zmr_all['Mstel'],jades_zmr_all['12 + log(O/H)'],
           marker="x",alpha=0.3,color='k',zorder=998)

chmrynska_zmr = plot_MZR.Chemerynska2024_MZR(tgt_zed,zed_tol=0.5)
ax.scatter(chmrynska_zmr['Mstel'],chmrynska_zmr['12 + log(O/H)'],label="Chemerynska+24 6<z<8 (UNCOVER)",
           color='k',marker="H",zorder=999)

nkjma = plot_MZR.Nakajima2023_MZR(tgt_zed)
ax.errorbar(10**nkjma["log_Mstel"],nkjma["12 + log(O/H)"],
            xerr=[10**(nkjma["log_Mstel"]-nkjma["log_Mstel err"]),10**(nkjma["log_Mstel"]+nkjma["log_Mstel err"])],
            yerr=[nkjma["12 + log(O/H) err"]],
            marker='*',color='k',ls='',lw=0.5,
            label='Nakajima+23 (binned) 4<z<10 (ERO/GLASS/CEERS)',zorder=999)
