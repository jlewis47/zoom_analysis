...
for isim, sim_dir in enumerate(sim_dirs):
    ...
    for igal in range(len(hids)):
        ...
        # dense_gas = gas["density"] / 1.67e-24 > 0.1
        # dense_gas = np.full_like(gas["density"], True)
        ...
        #     (
        #         gas["chem_O"]
        #         * sim.unit_d(1 / (sim_zed + 1))
        #         / 16.0
        #         / gas["density"] #replace chem_H
        #         / 0.02
        #         # / 0.76
        #     ),
        #     weights=gas["density"],
        ...
        cell_vols_cMpc3 = (sim.cosmo.lcMpc * 2 ** gas["ilevel"]) ** 3

        ZOH[igal] = np.sum(cell_vols_cMpc3 * gas["chem_O"] / 16.0) / np.sum(
            cell_vols_cMpc3 * gas["chem_H"] / 1.0
        )
        # ) / np.sum(cell_vols_cMpc3 * gas["density"] * 0.76)
        ...
        nfracO = 10 ** (lAO - lAH)
        mfracO = 10 ** (logO - lAH)
        conv_factor = mfracO / (1.0 - (XHAsplund + XHeAsplund))
...
ax.scatter(
    jades_zmr["Mstel"],
    jades_zmr["12 + log(O/H)"],
    label="Curti+24 3<z<10 (JADES)",
    color="k",
    marker="x",
    zorder=999,
)
...
ax.scatter(
    chmrynska_zmr["Mstel"],
    chmrynska_zmr["12 + log(O/H)"],
    label="Chemerynska+24 6<z<8 (UNCOVER)",
    color="k",
    marker="H",
    zorder=999,
)
...
if len(nkjma["log_Mstel"]) > 0:
    ax.errorbar(
        10 ** nkjma["log_Mstel"],
        nkjma["12 + log(O/H)"],
        xerr=[
            10 ** (nkjma["log_Mstel"] - nkjma["log_Mstel err"]),
            10 ** (nkjma["log_Mstel"] + nkjma["log_Mstel err"]),
        ],
        ...
        ls="",
        lw=0.5,
        label="Nakajima+23 (binned) 4<z<10 (ERO/GLASS/CEERS)",
        zorder=999,
    )
