...
assert False, "not adapted to DM yet"
...
for sdir in sdirs:
    ...
    rmax_hagn = np.zeros(nstep_tree_hagn, dtype=np.float32)
    ...
    rmax_zoom = np.zeros(nsteps, dtype=np.float32)
    ...
    fout_h5 = os.path.join(datas_path, f"DM_morpho.h5")
    read_existing = True
    ...
    if read_existing:
        ...
        with h5py.File(fout_h5, "r") as f:
            ...
            saved_rmax_hagn = f["r50_hagn"][:]
            saved_time_hagn = f["time_hagn"][:]

        if len(saved_mDM_zoom) == len(sim_snaps):

            mDM_zoom = saved_mDM_zoom
            r50_zoom = saved_r50_zoom
            rmax_zoom = saved_rmax_zoom
    ...
    if not read_existing or overwrite:
        ...
        for istep, (aexp, time) in enumerate(zip(hagn_tree_aexps, hagn_tree_times)):
            ...
            mDM_hagn[istep] = gal_pties["mgal"][0]
        ...
        for istep, (snap, aexp, time) in enumerate(
            ...
            mfrac = 1.0 - sim_tree_gal_datas["m_father"][0][sim_gal_tree_arg] / 100.0
            ...
            mDM_zoom[istep] = gal_pties["mass"]
            r50_zoom[istep] = gal_pties["r50"]
            rmax_zoom[istep] = gal_pties["rmax"]
            mfrac_zoom[istep] = mfrac
            time_zoom[istep] = time
            aexp_zoom[istep] = aexp

            # print(snap, 1.0 / aexp - 1, mDM_zoom[istep])
    ...
    (l,) = ax[0, 0].plot(
        time_zoom / 1e3, mDM_zoom, ls=zoom_ls[zoom_style], lw=2.0, color=color
    )
    ...
    if not read_existing or overwrite:
        ...
        with h5py.File(fout_h5, "w") as dest:
            ...
            dest.create_dataset("mDM_zoom", data=mDM_zoom, compression="lzf")
            dest.create_dataset("r50_zoom", data=r50_zoom, compression="lzf")
            dest.create_dataset("rmax_zoom", data=rmax_zoom, compression="lzf")
            ...
            dest.create_dataset("mDM_hagn", data=mDM_hagn, compression="lzf")
            dest.create_dataset("r50_hagn", data=r50_hagn, compression="lzf")
            dest.create_dataset("rmax_hagn", data=rmax_hagn, compression="lzf")
...
# sfrs_leja = sfr_ridge_leja22(tgt_zed, mDM_zoom)
# ax[0].plot(mass_bins, sfrs_leja / 10.0 / mDM_zoom, "k--")
# ax[0].annotate(
#     mass_bins[0], sfrs_leja[0] / 10.0 / mDM_zoom, "0.1xMS of Leja+22", color="k"
# )
