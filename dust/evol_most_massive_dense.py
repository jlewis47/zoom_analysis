...
plt.suptitle(f"{frac:.1f}Rvir, >{dens_thresh:.1f}Hpcc, Mgal>{mlim:.1e}Msun")
...
for isim, sdir in enumerate(sdirs):
    ...
    colors.append(
        cmaps[isim](
            np.linspace(0, 1, len(find_snaps_with_gals(sim.snap_numbers, sim.path)))
        )
    )
...
for isim, sdir in enumerate(sdirs):
    ...
    last_snap = sim.snap_numbers.max()
    last_aexp = sim.get_snap_exps(last_snap)[0]
    last_zed = 1 / last_aexp - 1.0

    assoc_snaps = find_snaps_with_gals(sim.snap_numbers, sim.path)

    nsnaps = len(assoc_snaps)

    print("Last snap:", last_snap, "at z=", last_zed)

    zstr = f"{last_zed:.2f}".replace(".", "p")

    name = sdir.split("/")[-1]
    dust_file = os.path.join(sdir, f"dens_dust_data_{last_snap:d}.h5")

    print("Calculating dust properties")
    intID = int(name[2:].split("_")[0])

    mstel_zoom = np.zeros(nsnaps, dtype=np.float32)
    sfr10_zoom = np.zeros(nsnaps, dtype=np.float32)
    sfr100_zoom = np.zeros(nsnaps, dtype=np.float32)

    volumes = np.zeros(nsnaps, dtype=np.float64)
    mG_zoom = np.zeros(nsnaps, dtype=np.float32)
    mZ_zoom = np.zeros(nsnaps, dtype=np.float32)

    mdCs = np.zeros(nsnaps, dtype=np.float32)
    mdCl = np.zeros(nsnaps, dtype=np.float32)
    mdSs = np.zeros(nsnaps, dtype=np.float32)
    mdSl = np.zeros(nsnaps, dtype=np.float32)

    fdMg_zoom = np.zeros(nsnaps, dtype=np.float32)
    fdFe_zoom = np.zeros(nsnaps, dtype=np.float32)
    fdO_zoom = np.zeros(nsnaps, dtype=np.float32)
    fdSi_zoom = np.zeros(nsnaps, dtype=np.float32)
    fdC_zoom = np.zeros(nsnaps, dtype=np.float32)

    for isnap, snap in enumerate(assoc_snaps):

        if 1.0 / sim.get_snap_exps(snap)[0] - 1.0 > 3.7:
            continue

        print(f"Snap {isnap+1}/{nsnaps}")

        all_gal_dict = get_gal_props_snap(sim.path, snap)

        gal_args = (all_gal_dict["host purity"] > fpure_lim) * (
            all_gal_dict["mass"] > mlim
        )

        most_massive_hid = all_gal_dict["host hid"][gal_args][
            np.argmax(all_gal_dict["mass"][gal_args])
        ]
        most_massive_gid = all_gal_dict["gids"][gal_args][
            np.argmax(all_gal_dict["mass"][gal_args])
        ]

        _, gal_dict = get_gal_props_snap(sim.path, snap, most_massive_gid)

        tgt_pos = gal_dict["pos"]
        tgt_r = (
            get_halo_props_snap(sim.path, snap, hids=[most_massive_hid])[
                f"halo_{most_massive_hid:07d}"
            ]["rvir"]
            * 0.1
        )

        if not check_for_compressd(sim, snap):
            # if (
            #     load_amr
            ...
            cells = code_to_cgs(sim, snap, code_cells)
            ...
            datas = read_compressed_target(sim, snap, hid=most_massive_hid)
        ...
        mstel_zoom[isnap] = masses.sum()
        sfr10_zoom[isnap] = masses[ages < 10].sum() / 10
        sfr100_zoom[isnap] = masses[ages < 100].sum() / 100
        ...
        volumes[isnap] = np.sum(vols)

        gas_mass = cells["density"] * vols / msun_to_g

        mG_zoom[isnap] = np.sum(gas_mass)

        mdCs[isnap] = np.sum(cells["dust_bin01"] * gas_mass)

        mdCl[isnap] = np.sum(cells["dust_bin02"] * gas_mass)

        mdSs[isnap] = np.sum(cells["dust_bin03"] * gas_mass) / SioverSil

        mdSl[isnap] = np.sum(cells["dust_bin04"] * gas_mass) / SioverSil

        mZ_zoom[isnap] = np.sum(cells["metallicity"] * gas_mass)
        ...
        fdMg_zoom[isnap] = 1 - mdMg[isnap] / mMg
        fdFe_zoom[isnap] = 1 - mdFe[isnap] / mFe
        fdO_zoom[isnap] = 1 - mdO[isnap] / mO
        fdSi_zoom[isnap] = 1 - mdSi[isnap] / mSi
        fdC_zoom[isnap] = 1 - mdC[isnap] / mC

    # lines.append(Line2D([0, 1], [0, 1], ls="-", c=l.get_color()))
    lines.append(Line2D([], [], marker=markers[isim], c=colors[isim][isnap], ls=""))
    labels.append(name)
...
fig_depl.savefig(f"figs/depletion_evol.png")

for a_dtm in [ax_dtm, ax_dtmCs, ax_dtmCl, ax_dtmSis, ax_dtmSil]:
    a_dtm[0].legend(lines, labels, framealpha=0.0)
fig_dtm.savefig(f"figs/dtm_evol.png")
fig_dtmCs.savefig(f"figs/dtm_Csevol.png")
fig_dtmCl.savefig(f"figs/dtm_Clevol.png")
fig_dtmSis.savefig(f"figs/dtm_Sisevol.png")
fig_dtmSil.savefig(f"figs/dtm_Silevol.png")
