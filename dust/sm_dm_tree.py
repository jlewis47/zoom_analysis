...
def plot_stellar_mass(ax, sim_times, mstel_zoom, **kwargs):
    (l,) = ax.plot(
        sim_times / 1e3,
        mstel_zoom,
        ls="-",
...
def plot_total_dust_mass(ax, sim_times, md_zoom, **kwargs):
    (l,) = ax.plot(sim_times / 1e3, md_zoom, ls="-", **kwargs)
    return l


def plot_dust_to_gas_ratio(ax, sim_times, md_zoom, mG_zoom, **kwargs):
    (l,) = ax.plot(sim_times / 1e3, md_zoom / mG_zoom, ls="-", **kwargs)
    return l


def plot_dust_to_metal_ratio(ax, sim_times, md_zoom, mZ_zoom, **kwargs):
    (l,) = ax.plot(sim_times / 1e3, md_zoom / mZ_zoom, ls="-", **kwargs)
    return l


def plot_carbon_small(ax, sim_times, md_zoom, **kwargs):
    (l,) = ax.plot(
        sim_times / 1e3, md_zoom, ls=":", marker="$C$", markersize=5, **kwargs
    )
    return l


def plot_carbon_large(ax, sim_times, md_zoom, **kwargs):
    (l,) = ax.plot(
        sim_times / 1e3, md_zoom, ls=":", marker="$C$", markersize=10, **kwargs
    )
    return l


def plot_silicate_small(ax, sim_times, md_zoom, **kwargs):
    (l,) = ax.plot(
        sim_times / 1e3, md_zoom, ls=":", marker="$S$", markersize=5, **kwargs
    )
    return l


def plot_silicate_large(ax, sim_times, md_zoom, **kwargs):
    (l,) = ax.plot(
        sim_times / 1e3, md_zoom, ls=":", marker="$S$", markersize=10, **kwargs
    )
    return l
...
for sdir in sdirs:
    ...
    plot_carbon_small(ax_carbon_small, tgt_times, md1_zoom, color=color)
    plot_carbon_large(ax_carbon_large, tgt_times, md2_zoom, color=color)
    plot_silicate_small(ax_silicate_small, tgt_times, md3_zoom, color=color)
    plot_silicate_large(ax_silicate_large, tgt_times, md4_zoom, color=color)
