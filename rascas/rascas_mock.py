...
def mock_spe_dump(
    ...
    with h5py.File(fout, "w") as f:
        ...
        gal_dat.create_dataset("galaxy_mass(msun)", data=gal_data[0])
        gal_dat.create_dataset("galaxy_sfr(msun.myr^-1)", data=gal_data[1])
        gal_dat.create_dataset("galaxy_ssfr(myr^-1)", data=gal_data[2])
        gal_dat.create_dataset("t(myr)", data=gal_data[3])
...
if __name__ == "__main__":
    ...
    model_name = "bc03_chabrier100_wlya_ndust"
    ...
    for gid in gids:
        ...
        rascas_dir = os.path.join(
            dsims, sim_id, "rascas", model_name, f"output_{snap:05d}", f"gal_{gid:07d}"
        )

        rascas_params = read_params(os.path.join(rascas_dir, "params_rascas.cfg"))
        ndir = int(rascas_params["mock"]["nDirections"])

        sim = ramses_sim(os.path.join(dsims, sim_id), nml="cosmo.nml")

        bricks = read_zoom_brick(snap, sim, hm)

        gal_arg = bricks["hosting info"]["hid"] == gid

        ctr_gal = [
            bricks["positions"]["x"][gal_arg],
            bricks["positions"]["y"][gal_arg],
            bricks["positions"]["z"][gal_arg],
        ]
        # rgal = bricks["virial properties"]["rvir"][gal_arg]
        rgal = bricks["smallest ellipse"]["r"][gal_arg]

        mgal = bricks["hosting info"]["hmass"][gal_arg]

        # ctr_gal = [0.63813084, 0.7364341052162953, 0.42615680278854773]
        # rgal = 0.0003
        # l = 100e3  # ckpc/h
        ...
        for idir in range(ndir):
            ...
            filt_names, mag_spec, mags, mag_errs = gen_spec(
                aexp, spec_obs_igm, lambda_bins, [names, wavs, trans]
            )
            ...
            with h5py.File(
                os.path.join(outp, f"mock_images_{sim_id}_{snap}_{gid}_{idir:d}.h5"),
                "w",
            ) as f:
            ...
            agegaltxt = f"age gal={stars['agepart'].max()/1e3:.2f} Gyr"
            masstxt = "M$_\star$=" + f"{mgal[0]:.2e}" + " M$_\odot$"
            ...
            fig_spe.savefig(
                os.path.join(outp, f"mock_spectrum_{sim_id}_{snap}_{gid}_{idir:d}.png")
            )
            fig_spe.savefig(
                os.path.join(outp, f"mock_spectrum_{sim_id}_{snap}_{gid}_{idir:d}.pdf")
            )
