...
start_aexp = avail_aexps.max()
pure = gprops["host purity"] > fpure
massive = gprops["mass"] > mlim
gids = gprops["gids"][pure * massive]

print(
    f'found {len(gids):d} galaxies more massive than {mlim:.2e} in {sim_path.split("/")[-1]}'
)
print(f"purity is {fpure:f} or greater")
print(
    f"starting at aexp={start_aexp:.2f}, z={1.0/start_aexp-1:.2f}, snap={sim.get_closest_snap(aexp=start_aexp)}"
)


tree_snaps_gids = np.full((len(sim_snaps), len(gids), 2), -1, dtype=np.int64)

for igal, target_gid in enumerate(gids):
    ...
    print(f"Setting up rascas for {sim_path.split('/')[-1]}, galaxy {target_gid}")
    ...
    for i, (snap, aexp, time) in enumerate(
        ...
        tree_snaps_gids[i, igal] = [cur_gid, snap]
        ...
        if (
            not os.path.exists(
                os.path.join(rascas_path, "RASCASDump", "rascas_dump.dat")
            )
            or overwrite
        ):
...
# trim zero rows in tree_snaps_gids # only works if one galaxy
...
with open(
    ...
    hdr_txt = "#-1 means empty/no progenitor\n#snap gid"
    for igal in range(tree_snaps_gids.shape[1]):
        hdr_txt += f" gal {igal:d}"
    dest.write(hdr_txt + "\n")
    for i in range(tree_snaps_gids.shape[0]):
        line_str = f"{tree_snaps_gids[i,0,1]:d}"
        for igal in range(tree_snaps_gids.shape[1]):
            line_str += f" {tree_snaps_gids[i,igal,0]:d}"
        dest.write(line_str + "\n")
