...
if __name__ == "__main__":
    ...
    for isnap, snap in enumerate(matching_snaps[::skip]):
        ...
        ax[1].plot(
            1.0 / wavs_lephare_ext * 1e4, ext_lephare_ext_calzetii, label="Calzetti"
        )

        # plt.tight_layout()
        zed_str = f"{zed:.2f}".replace(".", "p")
        fig.savefig(f"figs/test_att_{zed_str}.png")

        with open(f"fits/att_fit_{zed_str}.txt", "w") as f:
            np.savetxt(
                f,
                np.transpose(
                    [
