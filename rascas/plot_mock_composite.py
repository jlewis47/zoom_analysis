...
def plot_and_save_mock_composite(
    ...
    ax.plot([45, 45 + px_len_1as * 0.5], [20, 20], color="white", lw=2)
...
out_dir = os.path.join(
    "/data101/jlewis/color_mocks", model_name, sim_name, f"{snap:d}", f"gal_{gal:07d}"
)
...
for los_nb in los_nbs:
    ...
    mag_lims = [
        -2.5 * np.log10((10 ** (ml / -2.5)) / lim_area**2 * nircam_res_as[i] ** 2)
    ...
    for (fl_name, fl, fl_rebined, fl_rebined_psfd), ml in zip(
        ...
        ax.set_facecolor("black")
        fig.savefig(os.path.join(single_out, f"{fl_name}_{los_nb:d}_pretty.png"))
        fig, ax = plt.subplots(1, 1, figsize=(20, 20), layout="tight")
        i = ax.imshow(mag_rebinned.T, cmap="Greys", vmax=ml)
        plt.colorbar(i, ax=ax)
        ax.set_title(f"{fl_name}")
        ax.set_facecolor("black")
        fig.savefig(os.path.join(single_out, f"{fl_name}_{los_nb:d}_rebinned.png"))
        fig, ax = plt.subplots(1, 1, figsize=(20, 20), layout="tight")
        i = ax.imshow(mag_rebinned_psfd.T, cmap="Greys", vmax=ml)
        plt.colorbar(i, ax=ax)
        ax.set_title(f"{fl_name}")
        ax.set_facecolor("black")
        fig.savefig(os.path.join(single_out, f"{fl_name}_{los_nb:d}_rebinned_psfd.png"))
