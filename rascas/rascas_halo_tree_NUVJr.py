...
if __name__ == "__main__":
    ...
    # snap_targets=[207]
    snap_targets=np.arange(180,208)
    # gid=2
    # gid=4
    # snap_targets=[169]
    # snap = 203
    ...
    zoom_Mstars = np.zeros(len(sim_snaps))
    zoom_SFR = np.zeros(len(sim_snaps))
    zoom_sSFR = np.zeros(len(sim_snaps))
    zoom_NUV_r = np.zeros((len(sim_snaps),ndir))
    zoom_r_j = np.zeros((len(sim_snaps),ndir))
    zoom_F150W_F277W = np.zeros((len(sim_snaps),ndir))
    zoom_F277W_F444W = np.zeros((len(sim_snaps),ndir))
    ...
    for i, (snap, aexp, time) in enumerate(
        ...
        # aper_as_rf = (rgal * 2 * l * 1e3) * sim.cosmo_model.arcsec_per_kpc_comoving(
        #     0.0
        # ).value
        ...
        zoom_Mstars[i] = gal_mass
        zoom_SFR[i] = gal_sfr
        zoom_sSFR[i] = gal_ssfr
        ...
        for idir in range(ndir):
            ...
            tgt_nbins = int(2*rgal_ckpc / tgt_res)

            real_res = 2*rgal_ckpc / tgt_nbins
            # pixel_as = (
            #     (
            #         np.arctan(
            ...
            #     1.0, spec_rf, lambda_bins, [names, wavs, trans], rf=True
            # )
            ...
            r_pc = rgal_ckpc*1e3*aexp #physical parsecs
            ...
            _, band_ctrs, imgs_rf = gen_imgs(0.0, lambda_bins_rf, cube_rf, [names, wavs, trans], rpix = aper_px)           
            ...
            print("spec mags:", list(zip(names, mags_imgs_fnu)))
            print("spec mags rf:", list(zip(names, mags_imgs_fnu_rf)))
            ...
            zoom_NUV_r[i, idir] = mags_imgs_fnu_rf[nuv_arg] - mags_imgs_fnu_rf[r_arg]
            zoom_r_j[i, idir] = mags_imgs_fnu_rf[r_arg] - mags_imgs_fnu_rf[j_arg]
            zoom_F150W_F277W[i, idir] = mags_imgs_fnu[f150_arg] - mags_imgs_fnu[f277_arg]
            zoom_F277W_F444W[i, idir] = mags_imgs_fnu[f277_arg] - mags_imgs_fnu[f444_arg]
    ...
    time_colors = plt.cm.viridis((np.log10(sim_times)-np.log10(sim_times).min())/(np.log10(sim_times).max()-np.log10(sim_times).min()))
    ...
    fig.savefig('rascas_halo_tree_NUVJr.png')